...
(defn omit-nil
  "Takes a reducing function and returns a reducing function
   that only invokes the step arity for values that are not nil.

   Useful for aggregations where nil values should be excluded
   (e.g., sum, avg, count) rather than included."
  [rf]
  (fn
    ([] (rf))
    ([res] (rf res))
    ([res item] (if (some? item)
                  (rf res item)
                  res))))
